...
fn models() -> Vec<ModelInfo> {
    let mut models = vec![
        ...
        model("deepseek-v4.1-flash", 1000000, 384000),
        model("mimo-v2.5", 1000000, 128000),
        model("grok-4.5", 500000, 500000),
        model("grok-4.6", 500000, 500000),
        ...
        model("glm-5.3-flash", 1000000, 131072),
        ...
        model("hy3", 256000, 128000),
        model("hy4-preview", 1024000, 64000),
        model("longcat-2.0", 1000000, 131072),
        model("kimi-k2.6", 262144, 65536),
    ];
    for info in &mut models {
        info.effort_modes = match info.id.as_str() {
            "kimi-k3" => effort_modes(&["max"]),
            "deepseek-v4-flash" | "deepseek-v4.1-flash" => effort_modes(&["low", "high", "max"]),
            ...
            "glm-5.3" | "glm-5.3-flash" => effort_modes(&["low", "high", "max"]),
            "glm-5.2" => effort_modes(&["high", "max"]),
            "qwen3.8-max" | "qwen3.8-flash" => effort_modes(&["low", "medium", "xhigh"]),
            "hy3" => effort_modes(&["none", "low", "high"]),
            "hy4-preview" => effort_modes(&["none", "high"]),
            _ => Vec::new(),
        };
    }
    models
