...
int phase1_event_routing(Harness& harnessInst, TestState& state, const std::string& tempDir) {
    ...
    std::string prompt =
        "Write a fibonacci function to " + fibFile + " and test it with Python. "
        "The function should take an integer n and return the nth fibonacci number. "
        "Then run: python3 " + fibFile + " to verify it works with n=10.";
...

    {
        ...
        bool gotTool = state.events.cv.wait_for(lk, 60s, [&state] {
            return state.events.gotToolCall.load();
        });

        if (!gotTool) {
            std::cerr << "Phase 1 FAILED: ToolCallStarted not received within 60s" << std::endl;
            harnessInst.unsubscribe(subId);
            return EXIT_PHASE1_FAILED;
        }
        std::cout << "[Phase 1] ToolCallStarted received: " << state.events.capturedToolCallId << std::endl;
    }

    {
        std::unique_lock<std::mutex> lk(state.events.mtx);
        bool gotOutput = state.events.cv.wait_for(lk, 60s, [&state] {
            return state.events.gotProcessOutput.load();
        });

        if (!gotOutput) {
            std::cerr << "Phase 1 FAILED: ProcessOutputChunk not received within 60s" << std::endl;
            harnessInst.unsubscribe(subId);
            return EXIT_PHASE1_FAILED;
        }
        std::cout << "[Phase 1] ProcessOutputChunk received from PID: " << state.events.capturedProcessPid << std::endl;
    }

    {
        std::unique_lock<std::mutex> lk(state.events.mtx);
        bool gotChunk = state.events.cv.wait_for(lk, 30s, [&state] {
            return state.events.gotMessageChunk.load();
        });

        if (!gotChunk) {
            std::cerr << "Phase 1 FAILED: MessageChunk not received within 30s" << std::endl;
            harnessInst.unsubscribe(subId);
            return EXIT_PHASE1_FAILED;
        }
        std::cout << "[Phase 1] MessageChunk received (count: " << state.events.messageChunkCount << ")" << std::endl;
    }
...
int phase2_thread_switch(Harness& harnessInst, TestState& state, const std::string& tempDir) {
    ...
    std::cout << "[Phase 2] Current focused agent: " << harnessInst.focusedAgentId() << std::endl;
    ...
    std::cout << "[Phase 2] Current thread after creation: " << harnessInst.currentThreadId() << std::endl;

    const char* home = std::getenv("HOME");
    std::string lockPathA = std::string(home ? home : "/tmp") + "/.firmius/threads/" + state.threadA + "/.lock";
    if (!std::filesystem::exists(lockPathA)) {
        std::cerr << "Phase 2 FAILED: Thread-A lock file missing at " << lockPathA << std::endl;
        return EXIT_PHASE2_FAILED;
    }
    ...
    std::cout << "[Phase 2] Restored focused agent: " << restoredAgentId << std::endl;

    if (restoredAgentId != state.leadAgentId) {
        std::cerr << "Phase 2 FAILED: focusedAgentId not restored! Expected: " << state.leadAgentId
                  << ", Got: " << restoredAgentId << std::endl;
        return EXIT_PHASE2_FAILED;
    }

    auto agent = AgentRegistry::instance().getAgent(state.leadAgentId);
    if (!agent) {
        std::cerr << "Phase 2 FAILED: Agent not in registry after switch" << std::endl;
        return EXIT_PHASE2_FAILED;
    }
    ...
    if (turnsAfter <= turnsBefore) {
        std::cerr << "Phase 2 FAILED: History did not grow after follow-up (" << turnsBefore
                  << " -> " << turnsAfter << ")" << std::endl;
        return EXIT_PHASE2_FAILED;
    }

    std::cout << "Phase 2 PASSED: Thread switch and agent persistence verified" << std::endl;
    return EXIT_SUCCESS_ALL;
}

int phase3_abort_mid_process(Harness& harnessInst, TestState& state, const std::string& /*tempDir*/) {
    std::cout << "\n=== PHASE 3: Abort Mid-Process ===" << std::endl;

    if (state.leadAgentId.empty()) {
        std::cerr << "Phase 3 FAILED: No lead agent from previous phases" << std::endl;
    ...
    {
        std::unique_lock<std::mutex> lk(state.events.mtx);
        gotToolCall = state.events.cv.wait_for(lk, 30s, [&gotSleepToolCall] {
            return gotSleepToolCall.load();
        });
    }
    ...
    if (!gotToolCall) {
        std::cout << "[Phase 3] WARNING: LLM did not generate process_execute within timeout" << std::endl;
        std::cout << "[Phase 3] Marking as INCONCLUSIVE (not a harness failure)" << std::endl;
        state.phase3Inconclusive = 1;
        return EXIT_SUCCESS_ALL;
    }

    std::cout << "[Phase 3] ToolCallStarted received, waiting for process to start..." << std::endl;
    ...
    if (blockingPids.empty()) {
        std::cout << "[Phase 3] WARNING: No blocking process found (may have finished quickly)" << std::endl;
    ...
    if (std::all_of(blockingPid.begin(), blockingPid.end(), ::isdigit)) {
        ...
        isNumericPid = true;
        if (::kill(pid, 0) != 0) {
            std::cerr << "Phase 3 FAILED: Process " << pid << " not alive before abort" << std::endl;
            return EXIT_PHASE3_FAILED;
        }
    ...
    if (isNumericPid && pid > 0) {
        if (::kill(pid, 0) == 0) {
            std::cerr << "Phase 3 FAILED: Process " << pid << " still alive after abort" << std::endl;
            return EXIT_PHASE3_FAILED;
        }
        std::cout << "[Phase 3] Process " << pid << " successfully killed" << std::endl;
    }

    agent = AgentRegistry::instance().getAgent(state.leadAgentId);
    if (!agent) {
        std::cerr << "Phase 3 FAILED: Agent removed from registry (should be interrupt, not destroy)" << std::endl;
        return EXIT_PHASE3_FAILED;
    }
...
int phase4_concurrent_send_race(Harness& harnessInst, TestState& state, const std::string& /*tempDir*/) {
    std::cout << "\n=== PHASE 4: Concurrent Send Race ===" << std::endl;

    if (state.leadAgentId.empty()) {
        std::cerr << "Phase 4 FAILED: No lead agent from previous phases" << std::endl;
    ...
    while (!thread1Done || !thread2Done) {
        if (std::chrono::steady_clock::now() - startWait > 45s) {
            std::cerr << "Phase 4 FAILED: Timeout waiting for threads (possible deadlock)" << std::endl;
            t1.detach();
            t2.detach();
            return EXIT_PHASE4_FAILED;
        }
    ...
    if (deadlockDetected) {
        std::cerr << "Phase 4 FAILED: Deadlock detected in concurrent send" << std::endl;
        return EXIT_PHASE4_FAILED;
    }
    ...
    auto agent = AgentRegistry::instance().getAgent(state.leadAgentId);
    if (!agent) {
        std::cerr << "Phase 4 FAILED: Agent lost after concurrent sends" << std::endl;
        return EXIT_PHASE4_FAILED;
    }

    std::cout << "Phase 4 PASSED: Concurrent send race survived without deadlock or crash" << std::endl;
    return EXIT_SUCCESS_ALL;
}

int phase5_forced_compaction(Harness& harnessInst, TestState& state, const std::string& /*tempDir*/) {
    std::cout << "\n=== PHASE 5: Forced Compaction ===" << std::endl;

    if (state.leadAgentId.empty()) {
        std::cerr << "Phase 5 FAILED: No lead agent from previous phases" << std::endl;
    ...
    std::cout << "[Phase 5] Sending message to trigger compaction..." << std::endl;
    harnessInst.send("This is a test message to trigger context compaction.");

    bool gotCompaction = false;
    {
        std::unique_lock<std::mutex> lk(state.events.mtx);
        gotCompaction = state.events.cv.wait_for(lk, 30s, [&state] {
            return state.events.gotCompaction.load();
        });
    }
    ...
    if (!gotCompaction) {
        std::cerr << "Phase 5 FAILED: AgentCompacting event not received within 30s (BUG-10 not fixed?)" << std::endl;
        return EXIT_PHASE5_FAILED;
    }
    ...
    std::cout << "Phase 5 PASSED: Forced compaction event received and agent functional" << std::endl;
    return EXIT_SUCCESS_ALL;
}

int phase6_cleanup_integrity(Harness& harnessInst, TestState& state, const std::string& /*tempDir*/) {
    ...
    std::cout << "[Phase 6] Thread-A lock exists before shutdown: " << (lockExistedBefore ? "yes" : "no") << std::endl;
    ...
    if (content.find("threadId") == std::string::npos) {
        std::cerr << "Phase 6 FAILED: last_session.json missing threadId" << std::endl;
        return EXIT_PHASE6_FAILED;
    }

    std::cout << "[Phase 6] last_session.json contains valid data" << std::endl;
    std::cout << "[Phase 6] Thread locks released (flock handles closed)" << std::endl;
    std::cout << "[Phase 6] Temp directories will be cleaned on exit (RAII)" << std::endl;
...
bool checkSandboxImage() {
    int result = std::system("docker image inspect firmius-sandbox:latest > /dev/null 2>&1");
    return result == 0;
}
...
int main() {
    ...
    if (!checkDockerAvailable()) {
        std::cerr << "FAILED: Docker is not available. This audit requires Docker." << std::endl;
        return EXIT_GENERAL_FAILURE;
    }
    std::cout << "✓ Docker available" << std::endl;

    if (!checkSandboxImage()) {
        std::cerr << "FAILED: Docker image 'firmius-sandbox:latest' not found." << std::endl;
    ...
    result = phase3_abort_mid_process(harnessInst, state, tempDir.path());
    if (result != EXIT_SUCCESS_ALL) {
    ...
    result = phase4_concurrent_send_race(harnessInst, state, tempDir.path());
    if (result != EXIT_SUCCESS_ALL) {
    ...
    result = phase5_forced_compaction(harnessInst, state, tempDir.path());
    if (result != EXIT_SUCCESS_ALL) {
    ...
    result = phase6_cleanup_integrity(harnessInst, state, tempDir.path());
    if (result != EXIT_SUCCESS_ALL) {
    ...
    std::cout << "\n========================================================" << std::endl;
    std::cout << "✅ CHAOS AUDIT PASSED - All phases completed successfully" << std::endl;

    if (state.phase3Inconclusive) {
        std::cout << "⚠️  Note: Phase 3 was INCONCLUSIVE (LLM non-cooperation, not a harness failure)" << std::endl;
    }

    std::cout << "\nPhase Summary:" << std::endl;
    std::cout << "  Phase 1 (Event Routing):      PASSED - ToolCallStarted, ProcessOutputChunk, MessageChunk received" << std::endl;
    std::cout << "  Phase 2 (Thread Switch):      PASSED - focusedAgentId restored from threadAgentMap_ (BUG-1 fix)" << std::endl;
    std::cout << "  Phase 3 (Abort Mid-Process):  " << (state.phase3Inconclusive ? "INCONCLUSIVE" : "PASSED") << " - Process killed, agent survives" << std::endl;
    std::cout << "  Phase 4 (Concurrent Send):    PASSED - No deadlock or crash under concurrent load" << std::endl;
    std::cout << "  Phase 5 (Forced Compaction):  PASSED - AgentCompacting event received (BUG-10 fix)" << std::endl;
    std::cout << "  Phase 6 (Cleanup Integrity):  PASSED - Locks released, session saved" << std::endl;
