...
---
  ...
  for (const auto &turn : ag->getContext().history->turns) {
    for (const auto &msg : turn.messages) {
      for (const auto &part : msg.content) {
        if (const auto *txt = std::get_if<TextContent>(&part)) {
          if (txt->text.find("DENY_STOP") != std::string::npos) {
...
TEST_F(AgentStopHookTest,
       BlockingAgentStopHookDoesNotRefireBeforeMeaningfulResumedTurn) {
  if (!hooks::ScriptRuntime::enabled()) {
    GTEST_SKIP() << "Luau hooks disabled in this build";
  }
...
action:
  kind: script
  body: "local iter = state.read('thread', 'promise.iteration') or 0; state.write('thread', 'promise.iteration', iter + 1); if iter == 0 then return outcome.block{ reminder = 'DENY_STOP' } end; return outcome.allow{}"
---
)";
  hookFile.close();
  WorkflowLoader::instance().init();
  ...
  provider_->setResponses({"__EMPTY__", "real resumed progress", "final completion"});

  ThreadMetadata metadata;
  metadata.title = "AgentStop blocked recovery";
  metadata.hostOptions.type = HostType::Local;
  metadata.hostIdentifier = "localhost";
  metadata.cwd = "/tmp";
  metadata.leadPersona = "coder";
  ...
  ASSERT_TRUE(waitForCondition([&]() {
    auto ag = AgentRegistry::instance().getAgent(agentId);
    return ag && !ag->isRunning() && !ag->isBooting();
  }, std::chrono::milliseconds(5000))) << "agent did not stop";

  auto iterationJson = hooks::HookState::instance().readJson(
      hooks::HookState::Scope::Thread, "promise.iteration", "agent-stop-test");
  ASSERT_TRUE(iterationJson.has_value());
  rapidjson::Document iterDoc;
  ASSERT_FALSE(iterDoc.Parse(iterationJson->c_str()).HasParseError());
  ASSERT_TRUE(iterDoc.IsNumber());
  EXPECT_EQ(static_cast<int>(iterDoc.GetDouble()), 2);

  EXPECT_EQ(provider_->streamCount(), 2);

  auto ag = AgentRegistry::instance().getAgent(agentId);
  ASSERT_NE(ag, nullptr);
  bool sawGateReminder = false;
  bool sawResumedProgress = false;
  bool sawWrappedGateReminder = false;
  bool sawWrappedWorkflowContinuation = false;
  for (const auto &turn : ag->getContext().history->turns) {
    for (const auto &msg : turn.messages) {
      for (const auto &part : msg.content) {
        if (const auto *txt = std::get_if<TextContent>(&part)) {
          if (txt->text.find("DENY_STOP") != std::string::npos) {
            sawGateReminder = true;
            sawWrappedGateReminder = true;
          }
          if (txt->text.find("real resumed progress") != std::string::npos) {
            sawResumedProgress = true;
          }
          if (txt->text.find("<FIRMIUS_SYSTEM_SIGNAL kind=\"workflow_continuation\"") !=
              std::string::npos) {
            sawWrappedWorkflowContinuation = true;
          }
        }
      }
    }
  ...
  EXPECT_TRUE(sawWrappedGateReminder);
  EXPECT_TRUE(sawResumedProgress);
  EXPECT_TRUE(sawGateReminder);
  EXPECT_TRUE(sawWrappedWorkflowContinuation);
}
...
TEST_F(AgentStopHookTest,
       ExternalPromiseHookRecoversAfterEmptyFollowUpTurn) {
  if (!hooks::ScriptRuntime::enabled()) {
    GTEST_SKIP() << "Luau hooks disabled in this build";
  }
...
return outcome.block({
  ...
  ThreadMetadata metadata;
  metadata.title = "Promise stop gate empty follow-up";
  metadata.hostOptions.type = HostType::Local;
  metadata.hostIdentifier = "localhost";
  metadata.cwd = "/tmp";
  metadata.leadPersona = "coder";
  ...
  ThreadManager tm((testHome_ / ".firmius" / "threads").string());
  const std::string threadId = tm.createThread(metadata);
  const std::string promisedAgentId = "promised-agent-empty";

  hooks::HookState::instance().bindThread(threadId);
  ASSERT_TRUE(hooks::HookState::instance().applyBatch(
      {
          {hooks::HookState::Scope::Thread, "promise.id", R"("pact-empty")", false},
          {hooks::HookState::Scope::Thread, "promise.task",
           R"("finish the promised task")", false},
          {hooks::HookState::Scope::Thread, "promise.brief",
           R"("finish the promised task")", false},
  ...
  ASSERT_TRUE(waitForCondition([&]() {
    auto ag = AgentRegistry::instance().getAgent(agentId);
    return ag && !ag->isRunning() && !ag->isBooting() &&
           provider_->streamCount() >= 5;
  }, std::chrono::milliseconds(10000)))
      << "promise gate did not recover after empty follow-up";

  auto promiseJson = hooks::HookState::instance().readJson(
      hooks::HookState::Scope::Thread, "promise", "promise-test-empty");
  ASSERT_TRUE(promiseJson.has_value());
  rapidjson::Document promise;
  ASSERT_FALSE(promise.Parse(promiseJson->c_str()).HasParseError());
  ASSERT_TRUE(promise.IsObject());
  ...
  ASSERT_NE(ag, nullptr);
  bool sawDenyReminder = false;
  bool sawWorkflowContinuation = false;
  for (const auto &turn : ag->getContext().history->turns) {
    for (const auto &msg : turn.messages) {
      for (const auto &part : msg.content) {
        if (const auto *txt = std::get_if<TextContent>(&part)) {
          if (txt->text.find("PROMISE DENIED") != std::string::npos) {
            sawDenyReminder = true;
          }
          if (txt->text.find("<FIRMIUS_SYSTEM_SIGNAL kind=\"workflow_continuation\"") !=
              std::string::npos) {
            sawWorkflowContinuation = true;
          }
        }
      }
    }
  ...
  EXPECT_TRUE(sawDenyReminder);
  EXPECT_TRUE(sawWorkflowContinuation);
}
