...
public:
    ...
    MOCK_METHOD(void, emitProcessSpawned, (const std::string&, const std::string&, const std::string&), (override));
    MOCK_METHOD(std::string, spawnProcess, (const std::string&, const std::string&, const std::string&, (const std::map<std::string, std::string>&)), (override));
...
protected:
    void SetUp() override {
        testPromptsDir = std::filesystem::temp_directory_path() / "firmius_subagent_test_prompts";
...
TEST_F(SubagentToolTest, validation_passes_for_valid_persona) {
    ...
    // We expect validation to pass, but the engine call to fail because the environment isn't set up.
    // However, we want to ensure it doesn't fail with the "Invalid persona" error.
    ToolResult result = tool.execute(input, toolCtx);
    if (!result.success) {
        EXPECT_THAT(result.error, ::testing::Not(::testing::HasSubstr("Invalid persona")));
    }
}
...
TEST_F(SubagentToolTest, concurrent_validation_requests) {
    ...
    auto runTest = [&]() {
        ...
        for (int i = 0; i < requestsPerThread; ++i) {
            ...
            // Only test the validation logic in concurrent test to avoid Engine interference
