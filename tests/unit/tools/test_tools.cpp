...
public:
    ...
    MOCK_METHOD(void, writeFile, (const std::string&, (const std::vector<uint8_t>&)), (override));
    ...
    MOCK_METHOD(void, registerBackgroundProcess, 
                ...
                (override));
    MOCK_METHOD(ProcessSnapshot, inspectBackgroundProcess, (const std::string&), (override));
    MOCK_METHOD(void, writeToBackgroundProcess, (const std::string&, const std::string&), (override));
...
public:
    ...
    MOCK_METHOD(void, run, (const std::string&, (std::function<void(const StreamEvent&)>)), (override));
    MOCK_METHOD((const AgentContext&), getContext, (), (const, override));
    MOCK_METHOD(AgentContext&, getMutableContext, (), (override));
    MOCK_METHOD(std::string, resolvePath, (const std::string&), (const, override));
    MOCK_METHOD(void, interrupt, (), (override));
    MOCK_METHOD(bool, isInterrupted, (), (const, override));
    MOCK_METHOD(void, setModel, (const std::string&, const std::string&), (override));
    MOCK_METHOD(bool, isRunning, (), (const, override));
    MOCK_METHOD(std::string, spawnProcess, 
                (const std::string&, const std::string&, 
                 (const std::map<std::string, std::string>&)), 
                (override));
    MOCK_METHOD(ProcessSnapshot, inspectProcess, (const std::string&), (override));
    MOCK_METHOD(void, writeToProcess, (const std::string&, const std::string&), (override));
...
public:
    MOCK_METHOD(void, onOutput, ((std::function<void(const std::string&, bool)>)), (override));
...
rapidjson::Value makeJsonString(const std::string& str, rapidjson::Document::AllocatorType& alloc) {
    return rapidjson::Value(str.c_str(), alloc);
}

rapidjson::Document createJsonInput(const std::map<std::string, std::string>& stringFields,
    ...
    for (const auto& [key, value] : stringFields) {
        doc.AddMember(makeJsonString(key, alloc), makeJsonString(value, alloc), alloc);
    }
...
protected:
    ...
    void SetUp() override {
        ...
        ON_CALL(mockAgent, getMutableContext()).WillByDefault(ReturnRef(agentContext));
        ON_CALL(mockAgent, resolvePath(_)).WillByDefault(Invoke([](const std::string& path) {
...
TEST_F(FileReadToolTest, allowedPaths_permitsInside) {
    ...
    ToolContext ctx{mockHost, mockAgent};
...
TEST_F(FileReadToolTest, lineSlicing) {
    ...
    auto json = createJsonInput({{"path", "file.txt"}}, {{"start_line", 2}, {"end_line", 4}});
    ToolContext ctx{mockHost, mockAgent};
...
TEST_F(GrepToolTest, binaryFilesSkipped) {
    ...
    EXPECT_CALL(mockHost, exec(HasSubstr("--binary-files=without-match"), _, _, _))
...
TEST_F(GrepToolTest, exitCode1_noMatches) {
    ...
    auto json = createJsonInput({{"pattern", "nonexistent_pattern_12345"}, {"path", "/tmp"}});
    ToolContext ctx{mockHost, mockAgent};
...
TEST_F(GrepToolTest, contextLines) {
    ProcessResult result;
    result.exitCode = 0;
    result.stdoutData = "/tmp/file.txt-5-Before context\n/tmp/file.txt:6:Match line\n/tmp/file.txt-7-After context\n";
    
    EXPECT_CALL(mockHost, exec(HasSubstr("-B 2"), _, _, _)).WillOnce(Return(result));
    
    auto json = createJsonInput({{"pattern", "test"}, {"path", "/tmp"}}, {{"context_before", 2}, {"context_after", 3}});
    ToolContext ctx{mockHost, mockAgent};
...
TEST_F(ProcessExecuteToolTest, cwdDefaultsToAgentCwd) {
    ...
    EXPECT_CALL(mockAgent, spawnProcess(_, _, _))
        .WillOnce(Invoke([&capturedCwd](const std::string&, const std::string& cwd, const auto&) {
            capturedCwd = cwd;
            return "proc_123";
        }));
...
TEST_F(ProcessExecuteToolTest, timeoutHandling) {
    EXPECT_CALL(mockAgent, spawnProcess(_, _, _)).WillOnce(Return("proc_123"));
