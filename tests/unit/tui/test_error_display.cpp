...
TEST(ErrorDisplayTest, RendersRollingNoticeWithMetadataAndActiveGlint) {
  firmius::shared::NoticeContent notice;
  notice.title = "Rolling Update";
  notice.message = "Compacting history...";
  firmius::shared::RollingNoticeMetadata rolling_meta{};
  rolling_meta.eventKind = "compaction";
  rolling_meta.lifecycle = "InProgress";
  rolling_meta.modelLabel = "gpt-4";
  rolling_meta.sourceTurnCount = 10;
  rolling_meta.summaryTokens = 500;
  rolling_meta.savedTokens = 1200;
  ...
  EXPECT_NE(output.find("[InProgress]"), std::string::npos);
  EXPECT_NE(output.find("compaction"), std::string::npos);
  EXPECT_NE(output.find("Model:"), std::string::npos);
  EXPECT_NE(output.find("gpt-4"), std::string::npos);
  EXPECT_NE(output.find("Turns:"), std::string::npos);
  EXPECT_NE(output.find("10"), std::string::npos);
  EXPECT_NE(output.find("Summary tokens:"), std::string::npos);
  EXPECT_NE(output.find("500"), std::string::npos);
  EXPECT_NE(output.find("Saved tokens:"), std::string::npos);
  EXPECT_NE(output.find("1200"), std::string::npos);
  EXPECT_NE(output.find("Compacting history..."), std::string::npos);
}

TEST(ErrorDisplayTest, RendersObservationStartAsCompactOneLinerWithSourceStat) {
  firmius::shared::NoticeContent notice;
  firmius::shared::RollingNoticeMetadata rolling_meta{};
  rolling_meta.eventKind = "Observation";
  rolling_meta.lifecycle = "Buffering";
  rolling_meta.sourceStartTurnId = "XY";
  rolling_meta.sourceEndTurnId = "AB";
  rolling_meta.sourceTokens = 16747;
  notice.rollingMetadata = rolling_meta;

  auto theme = firmius::tui::ThemeManager::instance().getCurrentTheme();
  auto output = renderToString(firmius::tui::RenderNoticeDisplay(theme, notice));

  EXPECT_NE(output.find("Observing turns XY .. AB..."), std::string::npos);
  EXPECT_NE(output.find("↑16747 src"), std::string::npos);
  EXPECT_EQ(output.find("ROLLING MEMORY"), std::string::npos);
  EXPECT_EQ(output.find("[ ACTIVE ]"), std::string::npos);
}

TEST(ErrorDisplayTest, RendersObservationCompleteAsCompactOneLinerWithOutputStat) {
  firmius::shared::NoticeContent notice;
  firmius::shared::RollingNoticeMetadata rolling_meta{};
  rolling_meta.eventKind = "observation";
  ...
  rolling_meta.sourceStartTurnId = "XY";
  rolling_meta.sourceEndTurnId = "AB";
  rolling_meta.summaryTokens = 653;
  notice.rollingMetadata = rolling_meta;

  auto theme = firmius::tui::ThemeManager::instance().getCurrentTheme();
  auto output = renderToString(firmius::tui::RenderNoticeDisplay(theme, notice));

  EXPECT_NE(output.find("Observed turns XY .. AB."), std::string::npos);
  EXPECT_NE(output.find("↓653 out"), std::string::npos);
  EXPECT_EQ(output.find("ROLLING MEMORY"), std::string::npos);
  EXPECT_EQ(output.find("[ ACTIVE ]"), std::string::npos);
}

TEST(ErrorDisplayTest, RendersObservationActivationAsCompactOneLinerWithoutStats) {
  firmius::shared::NoticeContent notice;
  firmius::shared::RollingNoticeMetadata rolling_meta{};
  rolling_meta.eventKind = "observation";
  rolling_meta.lifecycle = "activate";
  rolling_meta.sourceStartTurnId = "XY";
  rolling_meta.sourceEndTurnId = "AB";
  rolling_meta.sourceTokens = 100;
  rolling_meta.summaryTokens = 50;
  notice.rollingMetadata = rolling_meta;

  auto theme = firmius::tui::ThemeManager::instance().getCurrentTheme();
  auto output = renderToString(firmius::tui::RenderNoticeDisplay(theme, notice));

  EXPECT_NE(output.find("Activated observation for turns XY .. AB."),
            std::string::npos);
  EXPECT_EQ(output.find("↑100 src"), std::string::npos);
  EXPECT_EQ(output.find("↓50 out"), std::string::npos);
  EXPECT_EQ(output.find("ROLLING MEMORY"), std::string::npos);
}
