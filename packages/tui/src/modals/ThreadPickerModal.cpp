...
ftxui::Component ThreadPickerModal::create(TuiState &state) {
  ...
  auto threads_ptr = std::make_shared<std::vector<firmius::shared::ThreadMetadata>>();

  auto menu = ftxui::Menu(entries.get(), selected.get());

  auto modal_renderer = ftxui::Renderer(menu, [menu, entries, threads_ptr, &h] {
    auto current_threads = h.listThreads();
    if (threads_ptr->size() != current_threads.size()) {
  ...
  return ftxui::CatchEvent(
      modal_renderer, [threads_ptr, selected, &state, &h](ftxui::Event event) {
