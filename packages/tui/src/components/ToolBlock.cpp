...
static std::string phaseLabel(ToolPhase phase) {
    switch (phase) {
        case ToolPhase::Preparing: return "preparing";
        case ToolPhase::Called: return "called";
        case ToolPhase::Finished: return "finished";
    }
    return "unknown";
}
...
ftxui::Component ToolBlock(const std::shared_ptr<ToolCallView>& view) {
    ...
    return ftxui::Renderer(container, [view, toggle] {
        ...
        std::string head = "[tool] " + view->name + " (" + phaseLabel(view->phase) + ")";
        ...
        bool can_toggle = (view->phase == ToolPhase::Finished && !view->result.empty());
        ...
        for (auto& line : body_lines) rows.push_back(line);
