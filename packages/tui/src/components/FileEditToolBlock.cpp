...
static std::vector<size_t> rankHunksByRelevance(const std::vector<DiffHunk>& hunks) {
...
static std::vector<DiffHunk> parseDiff(const std::string& /*path*/, 
  ...
  std::vector<std::string> old_lines, new_lines;
  {
    ...
    std::string line;
    while (std::getline(ss, line)) old_lines.push_back(line);
  }
  {
    std::istringstream ss(new_str);
    std::string line;
    while (std::getline(ss, line)) new_lines.push_back(line);
  }
...
static ftxui::Element renderDiffHunk(const DiffHunk& hunk, bool compact = false) {
  ...
  for (const auto& [type, content] : hunk.lines) {
    ...
    while (line_num_str.size() < 4) line_num_str = " " + line_num_str;
    
    switch (type) {
      case '-':
        line_el = ftxui::hbox({
          ftxui::text(line_num_str + " ") | ftxui::dim | ftxui::color(ftxui::Color::RGB(150, 150, 150)),
          ftxui::text("− ") | ftxui::color(ftxui::Color::RGB(200, 80, 80)),
          ftxui::text(content) | ftxui::color(ftxui::Color::RGB(220, 100, 100))
        });
        line_num++;
        break;
      case '+':
        line_el = ftxui::hbox({
          ...
          ftxui::text(content) | ftxui::color(ftxui::Color::RGB(100, 220, 100))
        });
        break;
      case ' ':
        line_el = ftxui::hbox({
          ftxui::text(line_num_str + " ") | ftxui::dim | ftxui::color(ftxui::Color::RGB(100, 100, 120)),
          ftxui::text("  ") | ftxui::dim,
          ftxui::text(content) | ftxui::color(ftxui::Color::RGB(180, 180, 200))
        });
        line_num++;
        break;
    }
  ...
  return ftxui::Renderer(container, [view, toggle] {
    ...
    if (view->phase == ToolPhase::Preparing ||
      ...
      auto header = ftxui::hbox({
        ...
        ftxui::text(action + " ") | ftxui::bold | ftxui::color(ftxui::Color::RGB(200, 180, 100)),
        ftxui::text(filename) | ftxui::color(ftxui::Color::RGB(180, 160, 140))
      });

      // Show diff preview during Called if we have content
      if (view->phase == ToolPhase::Called && (!old_string.empty() || !new_string.empty())) {
        ...
        if (total_added > 0 || total_removed > 0) {
          ...
          if (total_added > 0)
            stats_parts.push_back(ftxui::text("+" + std::to_string(total_added)) | 
                                 ftxui::color(ftxui::Color::Green));
          if (total_removed > 0)
            stats_parts.push_back(ftxui::text("−" + std::to_string(total_removed)) | 
        ...
        return ftxui::vbox({
          ...
          ftxui::vbox(diff_elements) | ftxui::frame | ftxui::size(ftxui::HEIGHT, ftxui::LESS_THAN, 12),
          ftxui::hbox({stats, ftxui::filler(), ftxui::text(filename) | ftxui::dim})
        }) | ftxui::borderRounded | ftxui::color(ftxui::Color::RGB(160, 150, 130));
      }
    ...
    if (!view->success) {
      ...
      return ftxui::hbox({
        ...
        ftxui::text("Edit failed: " + err_msg) | ftxui::color(ftxui::Color::RedLight)
    ...
    int removed = 0, added = 0;
    {
      ...
      std::string line;
      while (std::getline(ss, line)) removed++;
    }
    {
      std::istringstream ss(new_string);
      std::string line;
      while (std::getline(ss, line)) added++;
    }
    ...
    rows.push_back(ftxui::hbox({
      ...
      ftxui::text(filename + " ") | ftxui::bold | ftxui::color(ftxui::Color::RGB(150, 230, 180)),
      ftxui::text(" [") | ftxui::dim,
      toggle->Render(),
      ftxui::text("]") | ftxui::dim,
      ftxui::filler(),
      removed > 0 ? (ftxui::text("−" + std::to_string(removed)) | ftxui::color(ftxui::Color::Red)) : ftxui::text(""),
      added > 0 ? (ftxui::text(" +" + std::to_string(added)) | ftxui::color(ftxui::Color::Green)) : ftxui::text(""),
    }));
    ...
    if (view->show_result && (!old_string.empty() || !new_string.empty())) {
      ...
      size_t maxHunks = expanded ? hunks.size() : 3; // Show top 3 hunks when collapsed
      size_t maxLines = expanded ? SIZE_MAX : static_cast<size_t>(UIState::instance().maxCollapsedLines);
      ...
      if (hunks.size() > maxHunks && !expanded) {
        ...
        if (hunks.size() > shownHunks) {
          ...
          diff_elements.insert(diff_elements.begin(),
            ftxui::text("  … " + std::to_string(hiddenCount) + " hunks hidden (press Ctrl+G to expand)") |
      ...
      if (diff_elements.empty() && is_overwrite) {
        ...
        int line_num = 1;
        size_t shown = 0;
        size_t lineLimit = expanded ? SIZE_MAX : static_cast<size_t>(UIState::instance().maxCollapsedLines);
        ...
        if (!expanded && line_num <= static_cast<int>(std::count(new_string.begin(), new_string.end(), '\n')) + 1) {
    ...
    return ftxui::vbox(rows) | ftxui::borderRounded | ftxui::color(ftxui::Color::RGB(140, 180, 160));
  });
}
