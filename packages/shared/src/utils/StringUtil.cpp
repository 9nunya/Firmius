...
size_t StringUtil::levenshteinDistance(std::string_view s1, std::string_view s2) {
    ...
    for (size_t i = 0; i <= s2.size(); ++i) v0[i] = i;
...
std::vector<size_t> StringUtil::findFuzzy(std::string_view text, std::string_view pattern, float threshold) {
    std::vector<size_t> matches;
    if (pattern.empty() || threshold > 1.0f) return matches;
    if (threshold <= 0.0f) {
        for (size_t i = 0; i <= text.size(); ++i) matches.push_back(i);
        return matches;
    }

    size_t n = pattern.length();
    if (text.length() < n) return matches;

    for (size_t i = 0; i <= text.length() - n; ++i) {
        std::string_view window = text.substr(i, n);
        size_t dist = levenshteinDistance(window, pattern);
        float similarity = 1.0f - (static_cast<float>(dist) / static_cast<float>(n));
...
std::string StringUtil::htmlToMarkdown(const std::string& html) {
    ...
    md = std::regex_replace(md, std::regex("<h4[^>]*>(.*?)</h4>", std::regex::icase), "#### $1\n");
    md = std::regex_replace(md, std::regex("<h5[^>]*>(.*?)</h5>", std::regex::icase), "##### $1\n");
    md = std::regex_replace(md, std::regex("<h6[^>]*>(.*?)</h6>", std::regex::icase), "###### $1\n");

    // Bold/Strong
    md = std::regex_replace(md, std::regex("<(b|strong)[^>]*>(.*?)</\\1>", std::regex::icase), "**$2**");

    // Italic/Em
    md = std::regex_replace(md, std::regex("<(i|em)[^>]*>(.*?)</\\1>", std::regex::icase), "*$2*");

    // Links
    md = std::regex_replace(md, std::regex("<a[^>]*href=\"([^\"]*)\"[^>]*>(.*?)</a>", std::regex::icase), "[$2]($1)");

    // Lists
    md = std::regex_replace(md, std::regex("<li[^>]*>(.*?)</li>", std::regex::icase), "- $1\n");
    md = std::regex_replace(md, std::regex("<ul[^>]*>(.*?)</ul>", std::regex::icase), "$1");
