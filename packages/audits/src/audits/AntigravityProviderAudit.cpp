...
std::string AntigravityProviderAudit::getId() const {
  return "antigravity_provider";
}

std::string AntigravityProviderAudit::getDescription() const {
  return "Antigravity provider tool-call audit";
}

shared::AuditResult
AntigravityProviderAudit::run(const std::vector<std::string> &) {
  AuditResult result;
  ...
  std::string cleanupCmd =
      "docker rm -f antigravity-audit-sandbox 2>/dev/null || true";
        ...
        std::visit(
            [&](auto &&e) {
              ...
              if constexpr (std::is_same_v<T, AgentError>) {
                ...
                std::ofstream out("/tmp/antigravity_request.json",
                                  std::ios::app);
                out << "\n--- TOOL CALL ---\n"
