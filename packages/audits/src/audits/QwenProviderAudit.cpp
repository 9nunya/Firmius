...
std::string QwenProviderAudit::getDescription() const { return "Qwen provider tool-call audit"; }
...
shared::AuditResult QwenProviderAudit::run(const std::vector<std::string>&) {
    ...
    std::string cleanupCmd = "docker rm -f qwen-audit-sandbox 2>/dev/null || true";
