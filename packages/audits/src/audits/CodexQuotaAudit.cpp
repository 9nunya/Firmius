...
std::string CodexQuotaAudit::getId() const { return "codex_quota"; }

std::string CodexQuotaAudit::getDescription() const { return "Show codex quota buckets"; }

shared::AuditResult CodexQuotaAudit::run(const std::vector<std::string>&) {
    AuditResult result;
    result.auditId = getId();
    std::cout << "Starting Codex Quota Audit..." << std::endl;
    Panic::init();
    EnvLoader::load(".env.local");
    auto& harness = Harness::instance();
    harness.init();
    std::cout << "\nFetching quotas for 'codex' provider..." << std::endl;
    auto quotas = harness.getAllQuotas("codex");
    std::cout << "\n--- Quota Display ---\n" << std::endl;
    if (quotas.empty()) {
