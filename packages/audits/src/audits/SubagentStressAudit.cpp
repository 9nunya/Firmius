...
std::string SubagentStressAudit::getDescription() const { return "Stress test subagent spawning"; }

shared::AuditResult SubagentStressAudit::run(const std::vector<std::string>& args) {
    AuditResult result;
    ...
    std::string threadId = harness.newThread(opts, "/home/nunya/Projects/Firmius", "firmius");
    if (threadId.empty()) {
            ...
            std::visit([&](auto&& e) {
                ...
                if constexpr (std::is_same_v<T, AgentError>) {
                    if (e.agentId == leadAgentId) {
                        std::cout << "\n[Event] AgentError (LEAD): " << e.message << std::endl;
                        done = true;
                        cv.notify_one();
                    } else {
                        std::cout << "\n[Event] AgentError (SUB): " << e.message << std::endl;
                    }
    ...
    if (subagentSpawns >= 2 && subagentCompletions >= 2) {
        std::cout << "\033[1;32mAUDIT PASSED\033[0m: Successfully spawned and completed multiple subagents." << std::endl;
        result.exitCode = 0;
        result.passed = true;
        return result;
    }
    std::cout << "\033[1;31mAUDIT FAILED\033[0m: Did not spawn and complete sufficient subagents." << std::endl;
    result.exitCode = 1;
    result.passed = false;
    return result;
}
