...
std::string HarnessChaosAudit::getDescription() const { return "Adversarial harness integrity audit"; }
...
int phase1_event_routing(Harness& harnessInst, TestState& state, const std::string& tempDir) {
    ...
    harnessInst.send(prompt);
    {
        ...
        bool gotTool = state.events.cv.wait_for(lk, 60s, [&state] { return state.events.gotToolCall.load(); });
        if (!gotTool) {
    ...
    {
        std::unique_lock<std::mutex> lk(state.events.mtx);
        bool gotOutput = state.events.cv.wait_for(lk, 60s, [&state] { return state.events.gotProcessOutput.load(); });
        if (!gotOutput) {
    ...
    {
        std::unique_lock<std::mutex> lk(state.events.mtx);
        bool gotChunk = state.events.cv.wait_for(lk, 30s, [&state] { return state.events.gotMessageChunk.load(); });
        if (!gotChunk) {
...
int phase2_thread_switch(Harness& harnessInst, TestState& state, const std::string& tempDir) {
    ...
    if (restoredAgentId != state.leadAgentId) {
        std::cerr << "Phase 2 FAILED: focusedAgentId not restored! Expected: " << state.leadAgentId << ", Got: " << restoredAgentId << std::endl;
        return EXIT_PHASE2_FAILED;
    }
    ...
    if (turnsAfter <= turnsBefore) {
        std::cerr << "Phase 2 FAILED: History did not grow after follow-up (" << turnsBefore << " -> " << turnsAfter << ")" << std::endl;
        return EXIT_PHASE2_FAILED;
    }
...
int phase3_abort_mid_process(Harness& harnessInst, TestState& state) {
    ...
    {
        std::unique_lock<std::mutex> lk(state.events.mtx);
        gotToolCall = state.events.cv.wait_for(lk, 30s, [&gotSleepToolCall] { return gotSleepToolCall.load(); });
    }
    harnessInst.unsubscribe(subId);
    if (!gotToolCall) {
...
int phase5_forced_compaction(Harness& harnessInst, TestState& state) {
    ...
    {
        std::unique_lock<std::mutex> lk(state.events.mtx);
        gotCompaction = state.events.cv.wait_for(lk, 30s, [&state] { return state.events.gotCompaction.load(); });
    }
    harnessInst.unsubscribe(subId);
    if (!gotCompaction) {
...
shared::AuditResult HarnessChaosAudit::run(const std::vector<std::string>& args) {
    AuditResult result;
