...
SearchResult GoogleSearchProvider::search(const std::string& query,
    ...
    if (!optAccount.has_value()) {
        errorResult.error = "No authenticated Antigravity accounts available";
        return errorResult;
    }
    ...
    // Uses googleSearch: {} in tools array, no functionDeclarations
    rapidjson::Document doc;
    doc.SetObject();
    auto& a = doc.GetAllocator();

    // Top-level Antigravity wrapper fields
    ...
    // Contents - single user turn with the query
    ...
    // Tools array - googleSearch and optionally urlContext
    // NO functionDeclarations - this is the key constraint
    ...
    // Generation config - temperature 0, topP 1 (matches TypeScript)
    ...
    // Wrap request in the top-level document
    ...
    if (response.code == 0) {
        errorResult.error = "Search request failed: " + response.error;
        return errorResult;
    }
