...
size_t BaseOpenAIProvider::headerCallback(char *ptr, size_t size, size_t nmemb,
                                          void *userdata) {
...
void BaseOpenAIProvider::processSSELine(
  ...
  if (d.HasMember("choices") && d["choices"].IsArray() &&
    ...
    if (choice.HasMember("finish_reason") &&
      ...
      if (fr == "tool_calls")
        ...
        sr = StopReason::MaxTokens;
      else if (fr == "content_filter")
        sr = StopReason::ContentFilter;
  ...
  if (d.HasMember("usage") && d["usage"].IsObject()) {
    ...
    std::uint32_t promptTokens =
        usage.HasMember("prompt_tokens") && usage["prompt_tokens"].IsUint()
            ? usage["prompt_tokens"].GetUint()
            : 0;
    am.tokens.contextSize = promptTokens;
    am.tokens.completion = usage.HasMember("completion_tokens") &&
                                   usage["completion_tokens"].IsUint()
                               ? usage["completion_tokens"].GetUint()
                               : 0;
...
std::vector<ModelInfo> BaseOpenAIProvider::listModels() {
  ...
  if (d.IsObject() && d.HasMember("data") && d["data"].IsArray()) {
    for (const auto &m : d["data"].GetArray()) {
      ...
      else if (m.HasMember("max_context_length") &&
               m["max_context_length"].IsUint())
...
std::string
BaseOpenAIProvider::prepareRequestBody(const AgentHistory &history,
  ...
  if (!opts.tools.empty()) {
    ...
    for (const auto &t : opts.tools) {
      ...
      function.AddMember("description",
                         rapidjson::Value(t.description.c_str(), a), a);
      ...
      if (msg.role == Role::ToolResult) {
        ...
        for (const auto &part : msg.content) {
          if (auto *res = std::get_if<ToolResultContent>(&part)) {
            ...
            m.AddMember("tool_call_id",
                        rapidjson::Value(res->toolCallId.c_str(), a), a);
      ...
      for (const auto &part : msg.content) {
        if (auto *txt = std::get_if<TextContent>(&part)) {
          ...
          function.AddMember("arguments",
                             rapidjson::Value(tcc->args.c_str(), a), a);
...
bool BaseOpenAIProvider::supportsStreamUsage() const { return true; }
...
void BaseOpenAIProvider::calculateCost(AgentMetrics &metrics,
...
void BaseOpenAIProvider::generateSummary(
  ...
  finalMsg.AddMember("content", rapidjson::Value(compactionPrompt.c_str(), a),
                     a);
