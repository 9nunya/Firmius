...
std::string generateCodeChallenge(const std::string &verifier) {
  return base64UrlEncode(firmius::shared::sha256(verifier));
}
...
std::string buildAuthorizeUrl(const std::string &state,
  ...
  url << "&client_id=" << firmius::shared::urlEncode(kClientId);
  url << "&redirect_uri=" << firmius::shared::urlEncode(kRedirectUri);
  url << "&scope=" << firmius::shared::urlEncode(kScope);
  url << "&code_challenge=" << firmius::shared::urlEncode(challenge);
  url << "&code_challenge_method=S256";
  url << "&state=" << firmius::shared::urlEncode(state);
  url << "&id_token_add_organizations=true";
  url << "&codex_cli_simplified_flow=true";
  url << "&originator=" << firmius::shared::urlEncode(kOriginator);
  return url.str();
}
...
public:
  ...
  bool finalizeExchange(std::string &outErrorMessage) override {
    ...
    body += "&client_id=" + firmius::shared::urlEncode(kClientId);
    body += "&code=" + firmius::shared::urlEncode(authCode_);
    body += "&code_verifier=" + firmius::shared::urlEncode(verifier_);
    body += "&redirect_uri=" + firmius::shared::urlEncode(kRedirectUri);
...
bool CodexProvider::refreshAccessToken(OAuthAccount &acc) {
  ...
  body += "&refresh_token=" + firmius::shared::urlEncode(acc.refreshToken);
  body += "&client_id=" + firmius::shared::urlEncode(kClientId);
