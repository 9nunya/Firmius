...
void BenchmarkSession::ensureReady() {
    ...
    if (config_.personaName.empty()) {
        config_.personaName = "coder";
    }
    threadId_ = harness.newThread(config_.hostOptions, config_.cwd, config_.personaName);
    if (threadId_.empty()) {
        throw std::runtime_error("Failed to create benchmark thread");
    }
    agentId_ = Engine::instance().createAgent(threadId_, config_.personaName, false, "", "lead", "");
