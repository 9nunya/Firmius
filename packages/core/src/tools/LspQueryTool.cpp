...
shared::ToolMetadata LspQueryTool::getMetadata() const {
  return {"lsp",
          "Language-server operations: run semantic queries such as hover, definition, references, and symbols.",
          ToolScope::Semantic};
}

std::shared_ptr<shared::JSONSchema> LspQueryTool::getSchema() const {
  return zObject({
...
LspToolInput LspQueryTool::transform(const rapidjson::Value &json) {
  LspToolInput input;
...
shared::ToolResult LspQueryTool::execute(const LspToolInput &input,
