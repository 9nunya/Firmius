...
shared::ToolResult GrepTool::execute(const GrepInput& input, shared::ToolContext& ctx) {
    try {
        ...
        if (res.exitCode == 0) {
            ...
            // We search for the FIRST occurrence which separates the filename from the line number.
            // Then the second separator follows the line number.
            ...
            while (std::getline(stream, line)) {
                ...
                val.AddMember("content", rapidjson::Value(content.c_str(), a).Move(), a);
