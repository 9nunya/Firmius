...
void Agent::run(const std::string& task, std::function<void(const shared::StreamEvent&)> onEvent) {
    ...
    if (context.history->turns.empty()) {
        ...
        std::string protocolAddon = "\n\n# PROTOCOL STRICTNESS\n"
                                    ...
                                    "- If you emit any non-tool content, it is treated as your final response; do not include tool calls in that turn.\n";
        ...
        try {
            ...
            provider->stream(*context.history, opts, [&](const StreamEvent& ev) {
                ...
                if (auto* txt = std::get_if<TextChunk>(&ev)) {
                    ...
                    // Treat pure-whitespace content as non-visible so tools can still run.
            ...
            // If mixed content/tool output occurred, prefer tool calls and drop visible text.
            if (mixedContentAndTools && !accumulatedToolChunks.empty()) {
                if (debugPrettyPrint) {
                    std::cerr << "\033[1;33m[Mixed content and tool calls detected; discarding visible content]\033[0m\n";
                }
                fullResponse.clear();
                sawContent = false;
                ...
                if (mixedContentAndTools && debugPrettyPrint) {
                    std::cerr << "\033[1;33m[Mixed content and tool calls detected; discarding tool calls]\033[0m\n";
                }
            ...
            if (accumulatedToolChunks.empty()) {
                if (fullResponse.empty() && fullThinking.empty()) {
                    throw std::runtime_error("Provider returned empty response (timeout or model failure)");
                }
                taskFinished = true;
            } else {
                // --- State: ExecutingTool ---
