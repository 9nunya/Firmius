...
CLI, daemon and optional desktop installation, rejection of old daemon-less
...
select a newer release or build from source instead. Desktop remains optional
for installers and included in new release packages.
...
Ensure `${CARGO_HOME:-$HOME/.cargo}/bin` is on PATH. The desktop package can be
installed separately with `cargo install --locked --path apps/desktop`.